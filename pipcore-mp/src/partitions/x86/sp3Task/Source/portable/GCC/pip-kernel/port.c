...
static void prvSetInterruptGate(uint8_t ucNumber,
		ISR_Handler_t pxHandlerFunction, uint8_t ucFlags);
 ...
 */
extern void vPortTimerHandler(uint32_t esp);
...
static uint32_t ulSystemStack[configISR_STACK_SIZE] __attribute__((used))
= { 0 };
...
static void prvSetInterruptGate(uint8_t ucNumber,
	...
	xInterruptDescriptorTable[ucNumber].usISRLow = (uint16_t) (ulBase
			& USHRT_MAX);
	xInterruptDescriptorTable[ucNumber].usISRHigh = (uint16_t) ((ulBase >> 16UL)
			& USHRT_MAX);
...
static uint32_t lastScanCode;

void handlePageFault(uint32_t esp) {
...
void handleGPF(uint32_t esp) {

	printf("Got GPF, this sucks !!\r\n");
	vcli();
	for (;;)
		;
	__asm volatile("iret;");
...
BaseType_t xPortStartScheduler(void) {
	...
	if (pxCurrentTCB->typeOfTask) {
		printf("Starting normal task : %s at %x\r\n", pxCurrentTCB->pcTaskName,
