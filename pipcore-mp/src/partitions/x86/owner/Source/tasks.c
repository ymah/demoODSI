...
static void prvInitialiseTCBVariables(TCB_t * const pxTCB,
		const char * const pcName, UBaseType_t uxPriority,
		const MemoryRegion_t * const xRegions, const uint16_t usStackDepth) PRIVILEGED_FUNCTION; /*lint !e971 Unqualified char types are allowed for strings and single characters only. */
...
uint32_t xTaskPartitionCreate(uint32_t base, uint32_t length,
		uint32_t load_addr, TCB_t * pcTCB) {
	...
	if (createPartition((uint32_t) partitionEntry, (uint32_t) partpd,
		...
		printf("Partition creation error\r\n");
...
BaseType_t xTaskGenericCreate(TaskFunction_t pxTaskCode,
		...
		const MemoryRegion_t * const xRegions, taskType type) /*lint !e971 Unqualified char types are allowed for strings and single characters only. */
{
	BaseType_t xReturn;
	TCB_t * pxNewTCB;
	StackType_t *pxTopOfStack;

	printf("Creating task of type %d called %s with priority %d \r\n", type,
			pcName, uxPriority);
...
void vTaskDelay_ms(const TickType_t xTicksToDelay) {
    ...
    int i;
    for(i=0;i<2000*xTicksToDelay;i++);
    return ;
	printf("Asking for %dms delay\r\n",(uint32_t)xTicksToDelay);
	/* A delay time of zero just forces a reschedule. */
	if (xTicksToDelay > (TickType_t) 0U) {
...
eTaskState eTaskGetState(TaskHandle_t xTask) {
	...
	if (pxTCB == pxCurrentTCB) {
		...
		;
		{
			pxStateList = (List_t *) listLIST_ITEM_CONTAINER(
...
void vTaskStartScheduler(void) {
...
#else
	{
		...
		xReturn = xTaskCreate(prvIdleTask, "IDLE", tskIDLE_STACK_SIZE,
				(void * ) NULL, ( tskIDLE_PRIORITY | portPRIVILEGE_BIT ), NULL); /*lint !e961 MISRA exception, justified as it is not a redundant explicit cast to all supported compilers. */
	}
...
static TickType_t prvGetExpectedIdleTime(void) {
	...
	if (pxCurrentTCB->uxPriority > tskIDLE_PRIORITY) {
		xReturn = 0;
	} else if ( listCURRENT_LIST_LENGTH(
...
void vTaskSwitchContext(void) {
	//printf("Starting switch context\n");
	if (uxSchedulerSuspended != (UBaseType_t) pdFALSE) {
		...
		//printf("Check for stack overflow, if configured.\r\n");
...
#endif /* configUSE_NEWLIB_REENTRANT */
		//printf("Switch Newlibs _impure_ptr variable to point to the _reent structure specific to this task.\r\n");
	}
...
void vTaskPlaceOnEventListRestricted(List_t * const pxEventList,
		const TickType_t xTicksToWait, const BaseType_t xWaitIndefinitely) {
	TickType_t xTimeToWake;
...
BaseType_t xTaskCheckForTimeOut(TimeOut_t * const pxTimeOut,
	...
	;
	{
		...
		if ((xNumOfOverflows != pxTimeOut->xOverflowCount)
			...
			xReturn = pdTRUE;
		} else if ((xConstTickCount - pxTimeOut->xTimeOnEntering)
				< *pxTicksToWait) {
			...
			if (xExpectedIdleTime >= configEXPECTED_IDLE_TIME_BEFORE_SLEEP) {
				vTaskSuspendAll();
				{
					...
					if (xExpectedIdleTime
						...
						traceLOW_POWER_IDLE_BEGIN(); portSUPPRESS_TICKS_AND_SLEEP( xExpectedIdleTime ); traceLOW_POWER_IDLE_END();
					} else {
						mtCOVERAGE_TEST_MARKER();
					}
				}
...
static void prvInitialiseTCBVariables(TCB_t * const pxTCB,
		const char * const pcName, UBaseType_t uxPriority,
		const MemoryRegion_t * const xRegions, const uint16_t usStackDepth) /*lint !e971 Unqualified char types are allowed for strings and single characters only. */
{
	UBaseType_t x;
